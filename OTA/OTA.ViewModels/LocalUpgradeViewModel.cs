...
/// <summary>
/// 本地升级页面的状态与交互入口。
/// 承担串口刷新、槽位识别、升级准备与执行流程。
/// </summary>
public sealed class LocalUpgradeViewModel : ObservableObject
{
    ...
    public string ModeTitle => "STM32 本地 OTA 升级";

    public string ModeDescription => "流程：发送解锁 HEX -> 等待回包 -> 发送进入 Bootloader HEX -> 由 C# 内置 YMODEM 发送 STM32 程序。";

    public string ModeHint => "默认优先选择 USB-SERIAL CH340 串口。";

    public string PathLabel => "STM32程序路径";

    public string StartButtonText => "进入 Bootloader 并刷机";
    ...
    public PortOption? SelectedPort
    {
        ...
        set
        {
            if (SetProperty(ref _selectedPort, value) && value is not null)
            {
                ...
                PersistLocalUpgradePreferences();
            }
        }
    }
    ...
    {
        var preferences = _preferencesService.GetLocalUpgradePreferences();
    ...
    private void PersistLocalUpgradePreferences()
    {
        ...
        _preferencesService.SaveLocalUpgradePreferences(
            ScriptPath,
            _preferredPortName,
            BaudRateText);
    }
