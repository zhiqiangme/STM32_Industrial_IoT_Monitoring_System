...
internal sealed class PortDiscoveryService
{
    public async Task<IReadOnlyList<PortOption>> GetPortOptionsAsync()
    {
        ...
        var descriptions = await GetPortDescriptionsAsync();
        return portNames
            .Select(name =>
            {
    ...
    public PortOption? SelectPreferredPort(IReadOnlyList<PortOption> portOptions, string currentPortName, IReadOnlySet<string> knownPortNames)
    {
        ArgumentNullException.ThrowIfNull(portOptions);
        ArgumentNullException.ThrowIfNull(knownPortNames);

        var currentSelection = portOptions.FirstOrDefault(option =>
            string.Equals(option.PortName, currentPortName, StringComparison.OrdinalIgnoreCase));
        ...
        var newlyAddedPortNames = portOptions
            ...
            .Where(portName => !knownPortNames.Contains(portName))
    ...
    public static string NormalizePortName(string? rawText)
    {
        ...
        var match = Regex.Match(trimmed, @"COM\d+", RegexOptions.IgnoreCase);
        return match.Success ? match.Value.ToUpperInvariant() : trimmed;
    }

    private async Task<Dictionary<string, string>> GetPortDescriptionsAsync()
    {
    ...
    private static async Task<IReadOnlyList<string>> RunPowerShellAsync(string script, string? workingDirectory = null)
    {
        var wrappedScript = $$"""
