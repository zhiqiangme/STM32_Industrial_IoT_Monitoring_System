...
## 当前状态
- 已完成：服务端已回调 `G780S` 边缘 JSON 适配接入，恢复为透明传输链路。
- 已完成：服务端已新增 `relay-set` / `ota-prepare` 命令接口，并保留原有 MQTT / WebSocket / `cloud_ack` 链路。
- 已完成：服务端已补设备伪时间戳兜底，历史查询会优先落云端接收时间，避免 `HAL_GetTick` 导致时间轴异常。
- 已完成：STM32 固件当前代码已具备业务 JSON 上报、`cloud_ack` 等待、`relay_set` / `ota_prepare` 下行处理。
- 已完成：Flutter 已接入继电器控制页、Mill 仪表盘字段、历史字段改造、服务端接收时间兜底。
- 待验证：Flutter `analyze` 未跑完，本轮执行超时。
- 待验证：STM32 固件与 `pipe-monitor-api` 尚未做联调回归。

## Key Changes
- `PLAN4.md` 已完成项只回调边缘采集相关部分，不整单回退。
  - 回调服务端的 `G780S 边缘 JSON` 适配逻辑，也就是最近提交里新增的 `g780s_mapper.js` 和 `state.js` 中对边缘报文的映射接入。`[已完成]`
...
- `STM32_Mill` 固件改为“业务 JSON + OTA 保留”双态架构。
  - 日常业务态：`USART3` 面向 `G780S` 输出单行 JSON，上报测量值、状态、报警、继电器状态；下行接收控制命令。`[已完成]`
  - 下行最小命令固定为 `relay_set` 和 `ota_prepare`。`relay_set` 控制继电器；`ota_prepare` 负责停业务、回确认、进入升级准备态。`[已完成]`
...
- `D:\Project\HTML\server` 继续沿用现有 `pipe-monitor-api` 路线，不新建第二套后端。
  ...
  - 新增设备命令接口，由 API 发 MQTT 下行命令到设备；最小范围只做继电器控制和 OTA 准备。`[已完成]`
...
- 客户端沿用现有 Flutter 基线，不新起项目。
  ...
  - 页面保留现有数据看板思路，替换成 `Mill` 的字段与状态。`[已完成]`
  - 控制继电器走 API，不让手机直连 MQTT，不碰 OTA 工具链。`[已完成]`
