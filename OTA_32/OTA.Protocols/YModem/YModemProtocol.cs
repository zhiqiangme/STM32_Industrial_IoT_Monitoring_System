...
public static class YModemProtocol
{
    ...
    public static void SendFile(SerialPort serialPort, LocalUpgradeOptions options, byte[] image, uint crc32, string sha256Hex, Action<string> log)
    {
        ...
        while (sent < image.Length)
        {
            ...
            SendPacketWithRetry(serialPort, packet, TimeSpan.FromSeconds(options.TimeoutSeconds), expectCrcRequest: false);
    ...
    private static void SendPacketWithRetry(SerialPort serialPort, byte[] packet, TimeSpan timeout, bool expectCrcRequest)
    {
